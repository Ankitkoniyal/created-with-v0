...
export function SignupForm() {
  ...
  const checkEmailExists = async (email: string): Promise<{ exists: boolean; confirmed: boolean } | null> => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        // Fallback: let signup decide
        return null
      }

      debug("✅ Email check result:", data)
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      debug("🚀 Attempting signup for:", formValues.email)
      
      // RELIABLE email check - this will actually work now
      debug("📧 Checking if email exists via API...")
      const emailCheck = await checkEmailExists(formValues.email)
      if (emailCheck) {
        ...
        if (emailCheck.exists && !emailCheck.confirmed) {
          setError("This email is already registered but not verified. Please check your inbox for the verification link.")
          setIsSubmitting(false)
          return
        }
      }
      ...
      debug("✅ Signup successful:", data.user?.email)
