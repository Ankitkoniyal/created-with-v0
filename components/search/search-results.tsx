      ...
      try {
        ...
        if (filters.category) {
          ...
          const slug = rawCategory
            ...
            .replace(/\s+/g, '-')
            .replace(/&/g, 'and')
          // Match legacy rows (name) and new rows (slug), and optional category_slug
          const ors = [
            `category.eq.${slug}`,
            `category.eq.${rawCategory}`,
        ...
        if (filters.subcategory && filters.subcategory !== 'all') {
          const rawSub = String(filters.subcategory)
          const subSlug = rawSub
            .toLowerCase()
            .replace(/\s+/g, '-')
            .replace(/&/g, 'and')
          const ors = [
            `subcategory.eq.${subSlug}`,
            `subcategory.eq.${rawSub}`,
            `subcategory_slug.eq.${subSlug}`,
          ]
          query = query.or(ors.join(','))
        }
