...
export function DashboardNav() {
  ...
  const fetchDashboardData = async () => {
    ...
    try {
      ...
      const [adsResult, favoritesResult, messagesResult] = await Promise.allSettled([
      ...
      setDashboardCounts({
        myAds: adsResult.status === 'fulfilled' ? (adsResult.value.count || 0) : 0,
        favorites: favoritesResult.status === 'fulfilled' ? (favoritesResult.value.count || 0) : 0,
        messages: messagesResult.status === 'fulfilled' ? (messagesResult.value.count || 0) : 0,
      })
    } catch (error) {
  ...
  const getAvatarUrl = () => {
    if (userProfile?.avatar_url) {
      ...
      return `${userProfile.avatar_url}?t=${imageKey}`
    }
  ...
  const getMemberSinceText = () => {
    const date = userProfile?.created_at || user?.created_at
    if (date) {
