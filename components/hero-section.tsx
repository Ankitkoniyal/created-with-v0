...
export function HeroSection() {
  ...
  return (
    <section className="relative w-full py-8 sm:py-12 px-4 overflow-hidden bg-gradient-to-br from-gray-900 via-gray-950 to-black">
      ...
      <div className="relative max-w-6xl mx-auto text-center z-10">
        ...
        {/* UPDATED: Stats with Stronger Trust Indicator */}
        <div className={`flex flex-wrap justify-center gap-6 mb-4 transition-all duration-700 delay-200 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
          ...
          <div className="flex items-center gap-2 text-white">
            <ShieldCheck className="w-5 h-5 text-green-400" /> {/* Changed icon and color */}
            <span className="font-semibold">Verified</span>
            <span className="text-gray-400">Platform</span> {/* Replaced 98% Satisfaction */}
          </div>
        </div>

        {/* UPDATED: Feature Sections - Smaller Size, Catchier Copy, Larger Icons */}
        <div className={`grid grid-cols-1 md:grid-cols-3 gap-4 max-w-4xl mx-auto mb-10 transition-all duration-700 delay-300 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
          
          {/* Feature 1: Free Ads */}
          <div className="bg-white/5 backdrop-blur-md rounded-xl p-5 text-center border border-white/10 hover:border-green-400/30 transition-all duration-300 hover:scale-[1.03]">
            <div className="inline-flex items-center justify-center w-14 h-14 bg-green-700/30 rounded-full mb- mx-auto">
              <Upload className="w-7 h-7 text-green-400" /> {/* Larger Icon */}
            </div>
            <h3 className="font-bold text-white text-lg mb-1">Post for Free, Forever</h3> {/* Catchier title */}
            <p className="text-sm text-gray-300">Get unlimited visibility with 100% free ads to scale your business.</p> {/* Improved copy */}
          </div>

          {/* Feature 2: Website Links */}
          <div className="bg-white/5 backdrop-blur-md rounded-xl p-5 text-center border border-white/10 hover:border-green-400/30 transition-all duration-300 hover:scale-[1.03]">
            <div className="inline-flex items-center justify-center w-14 h-14 bg-green-700/30 rounded-full mb-3 mx-auto">
              <LinkIcon className="w-7 h-7 text-green-400" /> {/* Larger Icon */}
            </div>
            <h3 className="font-bold text-white text-lg mb-1">Direct Website Traffic</h3> {/* Catchier title */}
            <p className="text-sm text-gray-300">Boost your sales by including Direct Links back to your business site.</p> {/* Improved copy */}
          </div>

          {/* Feature 3: Video Integration */}
          <div className="bg-white/5 backdrop-blur-md rounded-xl p-5 text-center border border-white/10 hover:border-green-400/30 transition-all duration-300 hover:scale-[1.03]">
            <div className="inline-flex items-center justify-center w-14 h-14 bg-green-700/30 rounded-full mb-3 mx-auto">
              <Video className="w-7 h-7 text-green-400" /> {/* Larger Icon */}
            </div>
            <h3 className="font-bold text-white text-lg mb-1">Engaging Video Showcases</h3> {/* Catchier title */}
            <p className="text-sm text-gray-300">Bring your products to life with seamless YouTube Video integration.</p> {/* Improved copy */}
          </div>
        </div>

        {/* CTA Button (Unchanged, but looks great) */}
