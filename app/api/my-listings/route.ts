...
async function handleMyListingsRequest(req: Request) {\
  ...
  let user
  try {\
    const { data: { user: authUser }, error: userErr } = await withTimeout(
  ...
  try {\
    ...
    const { data, error } = await withTimeout(
      supabase
        .from("products")
        .select("id,title,description,price,status,category,created_at,primary_image,condition,location")
        .eq("user_id", user.id)
    ...
    const listings = (data || []).map((r: any) => ({
      ...
      adId: `AD${new Date(r.created_at).getFullYear()}${r.id.slice(-4).toUpperCase()}`,
