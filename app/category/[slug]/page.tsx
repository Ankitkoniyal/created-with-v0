      ...
      if (value && value !== "" && value !== "any") {
        switch (key) {
          ...
          case "negotiable":
            ...
            break
          case "kmDriven":
            filtered = filtered.filter((ad) => ad.kmDriven === value)
            break
          case "numberOfOwners":
            filtered = filtered.filter((ad) => ad.numberOfOwners === value)
            break
          case "manufacturingYear":
            filtered = filtered.filter((ad) => ad.manufacturingYear === Number(value))
            break
          case "transmissionType":
            filtered = filtered.filter((ad) => ad.transmissionType === value)
            break
          case "fuelType":
            filtered = filtered.filter((ad) => ad.fuelType === value)
            break
        }
      }
    })
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-600">
            {filteredAndSortedAds.length} {filteredAndSortedAds.length === 1 ? "item" : "items"} found
          </p>
        </div>
